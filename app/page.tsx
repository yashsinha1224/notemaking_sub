...
const HomePage = () => {
  ...
  const TEMP_USER_ID = "68b72fb98e98948ac07764bb"; // Use your actual user ID from the API response
  ...
  const fetchUser = async () => {
    try {
      ...
      // For now, just use the first user (you can modify this logic)
      if (users.length > 0) {
        setUser(users[0]);
      }
    } catch (error) {
  ...
  const handleDeleteNote = async (noteId) => {
    try {
      // For now, just remove from state (you can add DELETE API endpoint later)
      setNotes(notes.filter(note => note.noteId !== noteId));
      
      // Optional: Call DELETE API here when you implement it
  ...
  // Function to create a test note (for development)
